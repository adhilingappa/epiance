...
>
	...
	<!--Chome BluePrism Final 3 date:08/05/2020-->
...
public string getLastGuid(){
	...
	<xsl:variable name="stepDelay">3</xsl:variable>
	...
	<xsl:template  name="elementTemplateChrome">
		<xsl:element name="element">
			<xsl:attribute name="name">
				<xsl:value-of select="concat('Chr_',PlayBack--ControlInfo-ControlName,' element')"/>
			</xsl:attribute>
			<xsl:element name="id">
				<xsl:value-of select="Step-Guid"/>
			</xsl:element>
			...
			<xsl:element name="attributes">
				...
				<xsl:element name="attribute">
					...
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="PlayBack--ControlInfo-XPath!=''">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
				...
				<xsl:element name="attribute">
					...
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="PlayBack--ControlInfo-XPath='' and PlayBack--AdditionalInfo-Web_Name='' and PlayBack--AdditionalInfo-Web_ID!=''">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
				...
				<xsl:element name="attribute">
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="PlayBack--AdditionalInfo-Web_TagName"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
	...
	<xsl:template name="stepTemplate">
		<xsl:choose>
			<xsl:when test="PlayBack--ControlInfo-Role = 'combo box' or (PlayBack--ControlInfo-Role = 'list item' and PlayBack--AdditionalInfo-Web_TagName='SELECT')">
	...
	<xsl:template name="endStageTemplate">
		...
		<xsl:element name="stage">
			<xsl:variable name="tempEndGuid"><xsl:value-of select="utils:getLastGuid()"/></xsl:variable>
			<xsl:attribute name="stageid">
...
-->
	...
	<xsl:template name="matchDictionaryType">
		<xsl:choose>
			<xsl:when test="PlayBack--ControlInfo-Role='combo box'">WebList</xsl:when>
			<xsl:otherwise>
