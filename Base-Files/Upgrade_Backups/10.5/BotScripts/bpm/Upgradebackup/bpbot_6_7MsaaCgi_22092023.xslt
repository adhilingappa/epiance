...
>
			...
			<xsl:element name="attributes">
					...
					<xsl:if test="PlayBack--AdditionalInfo-UIA_AUTOMATIONID">
						<xsl:attribute name="inuse">
							<xsl:choose>
								...
								<xsl:when test="PlayBack--ControlInfo-ControlName = ''">
									<xsl:value-of select="$true"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$false"/>
				...
				<xsl:element name="attribute">
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="PlayBack--AdditionalInfo-UIA_ACCESSKEY"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				...
				<xsl:element name="attribute">
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="PlayBack--AdditionalInfo-UIA_PARENT_CLASSNAME"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				...
				<xsl:element name="attribute">
					...
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="PlayBack--ControlInfo-Role='combo box' and (PlayBack--AdditionalInfo-UIA_LOCALIZED_CONTROL_TYPE='button' or PlayBack--AdditionalInfo-UIA_LOCALIZED_CONTROL_TYPE='Button')">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
	...
	<xsl:template name="stepTemplate">
		<xsl:if test="PlayBack--AdditionalInfo-UIA_SCROLLITEMPATTREN='1'">
			<xsl:element name="step">
				<xsl:element name="element">
					<xsl:attribute name="id">
			...
			<xsl:choose>
				<xsl:when test="$waitForType='1'">
					<choices>
						...
						<ontrue><xsl:value-of select="utils:getGuid()"/></ontrue>
						<xsl:element name="element">
							<xsl:attribute name="id">
								<xsl:value-of select="Additional-guid1"/>
							</xsl:attribute>
	...
	<xsl:template name="matchDictionaryTemplate">
		<xsl:choose>
			...
			<xsl:when test="(PlayBack--ControlInfo-Role = 'push button') or (PlayBack--ControlInfo-Role = 'grid drop down button')">Button</xsl:when>
			...
			<xsl:when test="PlayBack--ControlInfo-Role = 'editable text'">Edit</xsl:when>
			...
			<xsl:when test="PlayBack--ControlInfo-Role = 'grid drop down button'">B</xsl:when>
			<xsl:when test="PlayBack--ControlInfo-Role = 'page tab'">TabItem</xsl:when>
			<xsl:when test="PlayBack--ControlInfo-Role = 'drop down button'">SplitButton</xsl:when>
			<xsl:when test="PlayBack--ControlInfo-Role = 'link'">Hyperlink</xsl:when>
	...
	<xsl:template name="matchDictionaryTemplate1">
		<xsl:choose>
			...
			<xsl:when test="PlayBack--ControlInfo-Role = 'list item' or PlayBack--AdditionalInfo-UIA_LOCALIZED_CONTROL_TYPE='list item'">UIAListItem</xsl:when>
