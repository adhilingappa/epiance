...
>
  ...
  <xsl:template name="selectorTemplateSAP"> 
      ...
      <xsl:if test="PlayBack--GeneralInfo-Event='TYPETEXT'">
        &lt;wnd app='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>' cls='<xsl:value-of select="PlayBack--AdditionalInfo-UIA_FOREGROUND_WINDOW_TYPE"/>' title='<xsl:value-of select="PlayBack--AppInfo-DialogName"/>' /&gt; &lt;sap id='<xsl:value-of select = "PlayBack--ControlInfo-UniqueID"/>'/&gt;
      </xsl:if>
    <xsl:if test="PlayBack--GeneralInfo-Event='Select' or PlayBack--GeneralInfo-Event='Click'">     
        &lt;wnd app='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>' cls='<xsl:value-of select="PlayBack--AdditionalInfo-UIA_FOREGROUND_WINDOW_TYPE"/>' title='<xsl:value-of select="PlayBack--AppInfo-DialogName"/>' /&gt; &lt;sap id='<xsl:value-of select = "PlayBack--ControlInfo-UniqueID"/>'/&gt;
     </xsl:if>
