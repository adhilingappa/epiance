...
<msxsl:script implements-prefix='incr' language='CSharp'>
 ...
 <xsl:variable name="ImagPath">
      ...
      <xsl:element name="ui:ClickImage.Image">
        <xsl:element name="ui:ImageTarget">
          ...
          <xsl:attribute name="TargetImageBase64">
            <xsl:value-of select="incr:ImageToBase64(concat($ImagPath,'\',ImageStore--ControlImage--ImageData-FileName))"/>
  ...
  <xsl:template name="selectorTemplateImageOLD">
    &lt; ctrl automationid='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>' idx='1' name='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>' role='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>'/&gt; &lt;
  </xsl:template> 
  
  <xsl:template name="selectorTemplateImage">
	  <xsl:if test="PlayBack--AppInfo-ExeName = 'iexplore.exe'">	
			  &lt;html title='<xsl:value-of select="incr:RemoveString(PlayBack--AppInfo-DialogName,' - Internet Explorer')"/>'/&gt; 
	  </xsl:if>
      <xsl:if test="PlayBack--AppInfo-ExeName = 'chrome.exe'">	
			  &lt;html app='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>' title='<xsl:value-of select="incr:RemoveString(PlayBack--AppInfo-DialogName,' - Google Chrome')"/>'/&gt; 
	  </xsl:if>
	  <xsl:if test="PlayBack--AppInfo-ExeName != 'chrome.exe' and PlayBack--AppInfo-ExeName != 'iexplore.exe'">	
			  &lt;wnd app='<xsl:value-of select="PlayBack--AppInfo-ExeName"/>' title='<xsl:value-of select="PlayBack--AppInfo-DialogName"/>'/&gt; 
	  </xsl:if>
