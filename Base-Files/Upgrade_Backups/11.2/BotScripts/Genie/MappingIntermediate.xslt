...
>
  ...
  <xsl:template match="/">
    ...
    <xsl:element name="mappingObj">     
      <xsl:for-each select="Document/Process/Step">
        <xsl:if test="PlayBack__ControlInfo_Role='editable text' and  PlayBack__GeneralInfo_Event='TYPETEXT'">
          <xsl:variable name="stpName">
            <xsl:value-of select="PlayBack__GeneralInfo_-Model--Properties-Name"/>
          </xsl:variable>
          <xsl:element name="{$stpName}">
             <xsl:value-of select="concat('formObj.','field_',Step-ID)"/>         
          </xsl:element>
        </xsl:if>
