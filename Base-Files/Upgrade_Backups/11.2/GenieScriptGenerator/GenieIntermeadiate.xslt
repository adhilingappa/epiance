  ...
  int iindex=0;
    ...
    public string replaceTabSpace(string controlName)
		...
		public string removeExe(string applicationName)
		{
			...
			string appName = MyRegex.Replace(applicationName, "");
			appName = Regex.Replace(applicationName, ".exe", "");
			return appName;
		}
	...
	<xsl:template name="RoboScript">
		...
		<xsl:element name="userData">
			...
			<xsl:if test="PlayBack__GeneralInfo_IsCitrix='1'">
			...
			<xsl:element name="OutputVar">
				<xsl:if test="PlayBack__GeneralInfo_Event='WindowActivate'">
					<xsl:value-of select="concat('ObjIE',',image',',',concat('Msaa_',utils:removeExe(PlayBack__AppInfo_ExeName)),',',concat('SAP_',utils:removeExe(PlayBack__AppInfo_ExeName)),',',concat('JAVA_',utils:removeExe(PlayBack__AppInfo_ExeName)))"/>
				</xsl:if>
				<xsl:if test="PlayBack__GeneralInfo_Event!='WindowActivate'">
					<xsl:value-of select="''"/>
				</xsl:if>
