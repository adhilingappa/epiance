...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:utils="urn:my-scripts" exclude-result-prefixes="msxsl utils">
	...
	<xsl:variable name="webSelectListItem">WebSelectListItem</xsl:variable>
	<xsl:variable name="webClick">WebClick</xsl:variable>
	<xsl:variable name="webSelect">WebSelect</xsl:variable>
	<xsl:variable name="itemindex">itemindex</xsl:variable>
	<xsl:variable name="flag">flag</xsl:variable>
	<xsl:variable name="itemtext">itemtext</xsl:variable>
	<xsl:variable name="webElement">WebElement</xsl:variable>
	...
	<xsl:template match="/">
		<xsl:element name="process">
			<xsl:attribute name="name">
				<xsl:value-of select="concat($GpsFile,'_chrome')"/>
			</xsl:attribute>
					...
					<xsl:for-each select="Document/Process/Step">
						<xsl:choose>
							<xsl:when test="PlayBack__GeneralInfo_Event = 'Click' or PlayBack__GeneralInfo_Event = 'Select' or PlayBack__GeneralInfo_Event='TYPETEXT'">
								<xsl:call-template name="elementTemplateChrome"/>
				...
				<xsl:choose>
					<xsl:when test="PlayBack__GeneralInfo_Event = 'Click' or PlayBack__GeneralInfo_Event = 'Select'">
						...
						<xsl:if test="utils:ToLower(Pre_Condition_Control_visible) = 'yes' and utils:ToLower(Include_Pre_Condition_Control_visible) = 'true'">
							...
							<xsl:call-template name="exceptionHandlingTemplate">
								<xsl:with-param name="exc" select="concat(BP_PlayBack__ControlInfo_ControlName, ' element not found')"/>
							</xsl:call-template>
						</xsl:if>
					...
					<xsl:when test="PlayBack__GeneralInfo_Event='TYPETEXT'">
						...
						<xsl:call-template name="exceptionHandlingTemplate">
							<xsl:with-param name="exc" select="concat('value in element does not match value typed in element - ',BP_PlayBack__ControlInfo_ControlName)"/>
						</xsl:call-template>
						<xsl:call-template name="calculationTemplate"/>
						<xsl:call-template name="endStageTemplate"/>
						<!--Read action-->
						...
						<xsl:call-template name="exceptionHandlingTemplate">
							<xsl:with-param name="subsheetId" select="Additional_guid1"/>
							<xsl:with-param name="exc" select="concat('empty value read from element ',BP_PlayBack__ControlInfo_ControlName)"/>
						</xsl:call-template>
						<xsl:call-template name="calculationTemplate">
							<xsl:with-param name="subsheetId" select="Additional_guid1"/>
						</xsl:call-template>
	...
	<xsl:template name="elementTemplateChrome">
		<xsl:element name="element">
			<xsl:attribute name="name">
				<xsl:value-of select="concat('Chr_',BP_PlayBack__ControlInfo_ControlName,' element')"/>
			</xsl:attribute>
			<xsl:element name="id">
				<xsl:value-of select="Step_Guid"/>
			</xsl:element>
			<xsl:element name="type">
				<xsl:call-template name="matchDictionaryType"/>
			</xsl:element>
			...
			<xsl:element name="attributes">
				<xsl:element name="attribute">
					<xsl:attribute name="name">wXPath</xsl:attribute>
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="Include_BP_PlayBack__ControlInfo_XPath='False'">
								<xsl:value-of select="$false"/>
							</xsl:when>
							<xsl:when test="Include_BP_PlayBack__ControlInfo_XPath='True'">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:when test="BP_PlayBack__ControlInfo_XPath!=''">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="BP_PlayBack__ControlInfo_XPath"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				<xsl:element name="attribute">
					<xsl:attribute name="name">wPageAddress</xsl:attribute>
					<xsl:attribute name="inuse">
						<xsl:choose>
				...
				<xsl:element name="attribute">
					<xsl:attribute name="name">wId</xsl:attribute>
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="Include_BP_PlayBack__AdditionalInfo_Web_ID='False'">
								<xsl:value-of select="$false"/>
							</xsl:when>
							<xsl:when test="Include_BP_PlayBack__AdditionalInfo_Web_ID='True'">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:when test="BP_PlayBack__ControlInfo_XPath='' and BP_PlayBack__AdditionalInfo_Web_Name='' and BP_PlayBack__AdditionalInfo_Web_ID!=''">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="BP_PlayBack__AdditionalInfo_Web_ID"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				<xsl:element name="attribute">
					<xsl:attribute name="name">wName</xsl:attribute>
					<xsl:attribute name="inuse">
						<xsl:choose>
							...
							<xsl:when test="BP_PlayBack__AdditionalInfo_Web_Name!=''">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
				...
				<xsl:element name="attribute">
					<xsl:attribute name="name">wElementType</xsl:attribute>
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="Include_PlayBack__AdditionalInfo_Web_TagName='False'">
								<xsl:value-of select="$false"/>
							</xsl:when>
							<xsl:when test="Include_PlayBack__AdditionalInfo_Web_TagName='True'">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$false"/>
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="PlayBack__AdditionalInfo_Web_TagName"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				<xsl:element name="attribute">
	...
	<xsl:template name="elementTemplateUia">
		<xsl:element name="element">
			<xsl:attribute name="name">
				<xsl:value-of select="concat('uia_',BP_PlayBack__ControlInfo_ControlName)"/>
			</xsl:attribute>
			<xsl:element name="id">
				<xsl:value-of select="utils:guid()"/>
			</xsl:element>
	...
	<xsl:template name="elementWindowTemplate">
		<xsl:element name="element">
			<xsl:attribute name="name">
				<xsl:value-of select="concat('uia_',BP_PlayBack__ControlInfo_ControlName,' elementWindow')"/>
			</xsl:attribute>
			<xsl:element name="id">
			...
			<xsl:element name="attributes">
				<xsl:element name="attribute">
					...
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="Include_BP_PlayBack__AppInfo_DialogName='False'">
								<xsl:value-of select="$false"/>
							</xsl:when>
							<xsl:when test="Include_BP_PlayBack__AppInfo_DialogName='True'">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$true"/>
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="BP_PlayBack__AppInfo_DialogName"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				<xsl:element name="attribute">
					...
					<xsl:attribute name="inuse">
						<xsl:choose>
							<xsl:when test="Include_BP_PlayBack__AdditionalInfo_UIA_FOREGROUND_WINDOW_TYPE='False'">
								<xsl:value-of select="$false"/>
							</xsl:when>
							<xsl:when test="Include_BP_PlayBack__AdditionalInfo_UIA_FOREGROUND_WINDOW_TYPE='True'">
								<xsl:value-of select="$true"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$true"/>
					...
					<xsl:element name="ProcessValue">
						...
						<xsl:attribute name="value">
							<xsl:value-of select="BP_PlayBack__AdditionalInfo_UIA_FOREGROUND_WINDOW_TYPE"/>
						</xsl:attribute>
					</xsl:element>
				</xsl:element>
				<xsl:element name="attribute">
...
&lt;![CDATA[FormViewer.Forms formObj = new FormViewer.Forms();</xsl:text>
				<xsl:value-of select="concat('dynamic Data1=formObj.ShowDVF(@&quot;',Document/Meta/RepositoryPath,'\BotScripts\Blue Prism\Version 6.7\',$GpsFile,'\',Document/Meta/ExternalBotFileName,'Forms.efm&quot;,null,1920,1080);')"/>
				<xsl:text disable-output-escaping="yes">
if(Data1!=null)
{
...
else
{
	...
	<xsl:template name="readTemplate">
		...
		<xsl:element name="stage">
			...
			<xsl:attribute name="name">
				<xsl:value-of select="concat('Read data from ',BP_PlayBack__ControlInfo_ControlName)"/>
			</xsl:attribute>
			...
			<xsl:element name="step">
				...
				<xsl:element name="action">
					<xsl:element name="id">
						<xsl:value-of select="'WebGetText'"/>
					</xsl:element>
				</xsl:element>
			</xsl:element>
		</xsl:element>
	...
	<xsl:template name="stepTemplate">
		<xsl:choose>
			<xsl:when test="PlayBack__ControlInfo_Role = 'combo box' or (PlayBack__ControlInfo_Role = 'list item' and PlayBack__AdditionalInfo_Web_TagName='SELECT')">
				<xsl:element name="step">
					<xsl:element name="element">
						<xsl:attribute name="id">
							<xsl:value-of select="Step_Guid"/>
					...
					<xsl:element name="action">
						<xsl:element name="id">
							<xsl:value-of select="$webSelectListItem"/>
						</xsl:element>
						...
						<xsl:element name="id">
							<xsl:choose>
								<xsl:when test="PlayBack__AdditionalInfo_Web_TagName='OPTION'">
									<xsl:value-of select="$webSelect"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="$webClick"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:element>
					</xsl:element>
			...
			<step>
				...
				<action>
					...
					<arguments>
						...
						<argument>
							<id>ProcessName</id>
							<value>"<xsl:value-of select="'chrome'"/>"</value>
						</argument>
						<argument>
							<id>ProcessID</id>
							<value>
	...
	<xsl:template name="exceptionHandlingTemplate">
		<xsl:param name="subsheetId" select="Step_Guid"/>
		<xsl:param name="exc" select="concat(BP_PlayBack__ControlInfo_ControlName, ' element exception')"/>
		<xsl:element name="stage">
			<xsl:attribute name="stageid">
	...
	<xsl:template name="matchDictionaryType">
		<xsl:choose>
			<xsl:when test="PlayBack__ControlInfo_Role='combo box'">WebList</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$webElement"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
